...
impl HypothesisTest for F {
        ...
        P: parameter_learning::ParameterLearning, 
    {
        ...
        return ((sum), alpha, all_within_limits); //p_value mean and limits p_value. | alpha always the same?
...
impl ChiSquare {
    ...
    pub fn compute_pvalue_chi_squared( 
        ...
        let ret = res.clone().into_iter().map(|x| x < (1.0 - self.alpha)).reduce(|a, b| a & b).unwrap_or(true); 
...
impl HypothesisTest for ChiSquare {
        ...
        P: parameter_learning::ParameterLearning,
    {
        ...
        return ((sum), alpha, all_within_limits);
    }
