...
pub struct Hiton2 <P: ParameterLearning>{
    parameter_learning: P,
    Ftest: F,
    Chi2test: ChiSquare,
}

impl<P: ParameterLearning> Hiton2 <P> {
    pub fn new(parameter_learning: P, Ftest: F, Chi2test: ChiSquare) -> Hiton2<P> {
        Hiton2 {
            parameter_learning,
            Ftest,
            Chi2test,
...
impl<P: ParameterLearning> StructuralLearningAlgorithm for Hiton2<P> { // Structural learning algorithm for hiton??? o altro??
                ...
                while candidate_parent_set_keys.len() > 0 {
                    ...
                    for separation_set_size in 1..= (CurrentPC.len() - 1){  // <-- Ciclo esterno che varia `separation_set_size`
                        ...
                        for X in CurrentPC.clone().iter() {
                            ...
                            let separation_set: BTreeSet<usize> = CurrentPC
                                .clone()
                                .into_iter() // Use into_iter() to get owned values for collecting
                                .filter(|&x| x != *X) // Filter out the current X
                                .take(separation_set_size as usize) // Take only 'separation_set_size' elements
                                .collect();
